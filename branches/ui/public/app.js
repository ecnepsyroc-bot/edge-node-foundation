        ...
        if (data.type === 'auth_success') {
            ...
            if (data.job_id === currentJobId) {
                ...
                if (subcategoryMatch && chatMatch) {
                    ...
                    displayMessage(data.text, data.timestamp, isSent ? 'sent' : 'received', data.username, messageSubcat, data.id);
                }
            }
        ...
        data.messages.forEach(msg => {
            ...
            if (isProblemMessage) {
                ...
                if (showInGeneralAll || showInOriginalLocation) {
                    ...
                    displayMessage(msg.text, msg.timestamp, isSent ? 'sent' : 'received', msg.username, messageSubcat, msg.id);
                    displayedCount++;
                }
            } else {
...
function displayMessage(text, timestamp, type, username = '', subcategory = 'All', messageId = null) {
...
async function deleteProblemMessage(messageId, problemText) {
    if (!confirm(`Delete problem: "${problemText}"?\n\nThis will permanently remove this Seeking Solution entry.`)) {
        return;
    }
